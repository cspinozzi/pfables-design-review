...
export default function ProviderPaymentsPage() {
  ...
  const [payoutMethods, setPayoutMethods] = useState([
    { id: "payout-1", bankName: "Chase Bank", last4: "8742", accountType: "Checking", isDefault: true },
  ])
  ...
  const [payments, setPayments] = useState([
    ...
    },
    {
      ...
      student: "Jake Wilson",
      parent: "Lisa Wilson",
      parentAvatar: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=100&h=100&fit=crop",
      ...
      <div className="page-container pt-6">
        ...
        <div className="mb-8">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Payments are deposited to your account within 3-5 business days after lesson completion.
          </p>
        </div>
        ...
        {filteredPayments.length > 0 ? (
          <div className="space-y-3">
            {filteredPayments.map((payment) => (
              <ServiceCard
                key={payment.id}
                image={payment.parentAvatar}
                imageAlt={payment.parent}
                title={payment.service}
                subtitle={`${payment.student} (${payment.parent})`}
          ...
          <Card className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {filter === "waiting"
                ? "No payments awaiting processing"
                : filter === "refunded"
        ...
        {selectedPayment && (
          <ServiceDetailModal
            ...
            subtitle={`${selectedPayment.student} (${selectedPayment.parent})`}
            status={selectedPayment.status}
            onStatusChange={(s) => handlePaymentStatusChange(selectedPayment.id, s as "waiting" | "done" | "requested" | "refunded")}
            people={[
              { name: selectedPayment.parent, role: "Parent", avatar: selectedPayment.parentAvatar },
              { name: selectedPayment.student, role: "Student" },
            ]}
            fields={[
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-amber-800">
                  {waiting.length} lesson{waiting.length > 1 ? "s" : ""} awaiting payment (${totalWaiting})
                </p>
                <p className="text-xs text-amber-700 mt-1">
                  Payments are typically processed within 3-5 business days after the parent pays.
                </p>
              </div>
            </div>
          </Card>
