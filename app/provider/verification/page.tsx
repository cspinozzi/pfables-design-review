...
export default function ProviderVerificationPage() {
  ...
  const [filesByCategory, setFilesByCategory] = useState<Record<string, UploadedFile[]>>({
    ...
    credentials: backgroundCheck?.documents
      .filter((d) => d.type === "credential")
      .map((d) => ({ name: d.name, size: "1.8 MB", type: "application/pdf", category: "credentials", previewUrl: d.fileUrl })) || [],
    background: [],
    additional: [],
  })
  ...
  const getProgressPercentage = () => {
    ...
    return Math.round((completed / required) * 80) // 80% max before admin review
  }
          ...
          <CardContent className="flex items-start gap-3 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Build trust with families, earn a verified badge, and rank higher in search
                results. All documents are reviewed within 2-3 business days.
              </p>
            </div>
          </CardContent>
        </Card>
