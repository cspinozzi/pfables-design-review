...
export default function RepairerMessagesPage() {
  ...
  const repairerConversations = conversations.filter((conv) =>
    conv.participants.some((p) => p.id === user?.id)
  )

  useEffect(() => {
    const convParam = searchParams.get("conv")
    if (convParam && repairerConversations.some((c) => c.id === convParam)) {
      setSelectedConversation(convParam)
    }
  }, [searchParams, repairerConversations])
  const selectedConv = repairerConversations.find((c) => c.id === selectedConversation)
    ...
    <div className="min-h-screen bg-background pb-20 sm:pb-0">
          ...
          <div className="h-full flex flex-col bg-background">
            <div className="border-b bg-background px-4 py-4 flex items-center justify-between">
              ...
              <div className="text-sm text-muted-foreground">
                {repairerConversations.length} {repairerConversations.length === 1 ? "conversation" : "conversations"}
              </div>
            </div>
              ...
              {repairerConversations.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-8 text-center h-full">
                  ...
                  <p className="text-sm text-muted-foreground max-w-sm">
                    When clients contact you about repairs, their messages will appear here
                  </p>
                </div>
              ) : (
      ...
      <div className="hidden md:block page-container py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-1 text-sm">Your conversations with clients</p>
        </div>
